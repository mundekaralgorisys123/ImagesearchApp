...
with open('model.txt','r') as file:
    feature_folder = file.read().strip()
feature_path_initialiser(feature_folder)
...
initialize_extractor('EfficientNetB0')
...
def model_training_task(folder_name):
    global training_status, features, img_paths
...
def train_similar():
    ...
    return render_template('train_similar.html',features=features,message1=message1,folder_names=folder_names)

def model_training_task1(folder_name):
...
def train():
    ...
    model_name = request.form.get('modelName')
    print(folder_name, model_name)
    initialize_extractor(model_name)
    feature_path_initialiser(model_name)
    thread = Thread(target=model_training_task1, args=(folder_name,))
    thread.start()
    return jsonify({'message': 'Model training started.'}), 200
